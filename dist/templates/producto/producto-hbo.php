<section class="Producto Producto--hbo">
  <div class="container">
    <div class="row Producto__wrapper-cols">
      <div class="col-lg-4 col-md-6 mob-order-1">
        <h2 class="Producto__title Producto__title--hbo">3x3x3</h2>
        <article class="Producto__text">
          <!-- <h4>¡La programación más exclusiva <span>será tuya!</span></h4> -->
          <p>Datos para navegar, redes sociales, llamadas y SMS. </p>
        </article>
        <a href="#form-promo" class="Producto__cta btn-g btn-blue-filled go-to">contratar</a>
      </div>
      <div class="col-lg-7 col-md-6 mob-order-2">
        <img src="assets/img/producto/netflix.png" alt="Producto" class="Producto__image" data-aos="zoom-in-left">
      </div>
    </div>
  </div>
</section>